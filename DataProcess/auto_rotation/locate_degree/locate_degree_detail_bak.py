...
def locate_degree_C001_C002(phase, freq):
    ...
    # level 4 ==> distance=4000
    peaks, _ = find_peaks(phase, height=0.3, distance=8000)
...
def locate_degree_F002_F003(phase, freq):
    # 定位 F002_F003 时间序列中的峰值，
    # freq < 922.625
    # 高峰，低峰，低峰，低峰 分别对应 degree_315，degree_45，degree_135, degree_225
    #
    # freq >= 922.625
    # 高峰，低峰，低峰 分别对应 degree_315，degree_45，degree_135
    #
    # level 2 ==> distance=6000
    # level 4 ==> distance=2500
    peaks, _ = find_peaks(phase, height=0.05, distance=6000)

    print('Before del, peaks: ', peaks)

    # 找到第一个高峰
    # 第一个高峰前可能有多个低峰，将其均移除
    while phase[peaks[0]] < phase[peaks[1]] or phase[peaks[0]] < 0.4:
        peaks = np.delete(peaks, [0])
    ...
    if freq < 922.625:
        # 第一个高峰对应 degree_315
        degree_315 = phase[peaks[0]]
        degree_45 = phase[peaks[1]]
        degree_135 = phase[peaks[2]]
        degree_225 = phase[peaks[3]]

        degree_360 = phase[peaks[0] + (peaks[1] - peaks[0]) // 2]
        degree_0 = phase[peaks[0] + (peaks[1] - peaks[0]) // 2]
        degree_90 = phase[peaks[1] + (peaks[2] - peaks[1]) // 2]
        degree_180 = phase[peaks[2] + (peaks[3] - peaks[2]) // 2]
        degree_270 = phase[peaks[3] + (peaks[4] - peaks[3]) // 2]
        ...
        degree_0 = phase[peaks[0] + (peaks[1] - peaks[0]) // 2]
        degree_90 = phase[peaks[1] + (peaks[2] - peaks[1]) // 2]
        degree_180 = phase[peaks[2] + (peaks[3] - peaks[2]) // 4]
        degree_225 = phase[peaks[2] + (peaks[3] - peaks[2]) // 2]
        degree_270 = phase[peaks[3] - (peaks[3] - peaks[2]) // 4]
